...
export const getReplyMessage = (userId, username, day) => {
  ...
  const messages = {
    ...
    5: `5 whole straight days later! You're doing amazing sweetie!! ${scrapbookLink}`,
    6: `You've almost made it to a week!!! Don't stop now! ${scrapbookLink}`,
    7: `A WHOLE WEEK OF STREAKS, you are on ffire :fire: ${scrapbookLink}`,
    "7+": `Hey queen! Girl, you have done it again, constantly raising the bar for us all, and doing it flawlessly. ${scrapbookLink}`,
  }
  if (day <= 7) {
    return messages[day]
  } else {
