...
Hack Clubbers can also use a `dino.icu` subdomain! You can use this domain for your Scrapbook or for another project. To get one, go to our [DNS repository](https://github.com/hackclub/dns), where we manage our DNS configuration through OctoDNS and there follow the instructions to obtain it, after that you can run `/scrappy setdomain <domain>` from instructions above.
