...
const errorPage = () => {
  ...
  let query = new URLSearchParams(router.asPath.substring(router.asPath.indexOf('?')))
  return <main>
    <Meta as={Head} name="Hack Club's Scrapbook" title="404" />
    <h1>{router.asPath.split("error=")[1] || "404!"} {router.asPath.split("error=")[1] && "Error"}</h1>
    <Link href="/">
      <a className="badge">
    ...
    <style>{`
      ...
      h1 {
        ...
        -webkit-text-stroke-width: ${router.asPath.split("error=")[1] ? '1px' : '2px'};
      ...
      @media (min-width: 32em) {
        h1 {
          -webkit-text-stroke-width: ${router.asPath.split("error=")[1] ? '2px' : '4px'};
